-- Reception of maintenance tickets ("en attente de devis") from an external field-service
-- tool: matches or creates the client/site/installation, then creates a local ticket ready
-- for quote creation. Separate opt-in and URL from the client/site/installation import.
alter table public.webhook_endpoints
  add column if not exists ticket_import_enabled boolean not null default false;
grant update (ticket_import_enabled) on public.webhook_endpoints to authenticated;
...
create index if not exists installations_owner_site_code_idx
  on public.installations(owner_id, site_id, code) where code is not null;

alter table public.tickets
  add column if not exists external_source text,
  add column if not exists external_ref text;
create unique index if not exists tickets_owner_external_idx
  on public.tickets(owner_id, external_source, external_ref) where external_ref is not null;

create function public.import_quote_ticket_from_webhook(p_token text, p_payload jsonb)
returns jsonb
