...
import type { QuoteTicketImportPayload, QuoteTicketImportReceipt } from "./quote-ticket-import";

export async function importQuoteTicket(
  token: string,
  payload: QuoteTicketImportPayload,
): Promise<QuoteTicketImportReceipt> {
  const url = process.env.SUPABASE_URL || import.meta.env.VITE_SUPABASE_URL;
  const key = process.env.SUPABASE_PUBLISHABLE_KEY || import.meta.env.VITE_SUPABASE_PUBLISHABLE_KEY;
  if (!url || !key) throw new WebhookError(503, "La création automatique n’est pas configurée.");
  const response = await fetch(new URL("/rest/v1/rpc/import_quote_ticket_from_webhook", url), {
    method: "POST",
    headers: {
      apikey: key,
