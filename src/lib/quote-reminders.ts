...
export function parseReminderSettings(value: unknown): ReminderSettings {
  ...
  return {
    ...
    delayDays:
      ...
      Number.isInteger(data.delayDays) &&
      data.delayDays >= 1 &&
      data.delayDays <= 365
        ? data.delayDays
        : 7,
...
export function getQuoteReminder(quote: ReminderQuote, settings: ReminderSettings, now: number) {
  if (!settings.enabled || !["envoye", "sent"].includes(quote.status) || !quote.sent_at)
    return null;
  ...
  const dueAt = Math.max(
    base + settings.delayDays * DAY_MS,
    Number.isFinite(snoozed) ? snoozed : 0,
  );
