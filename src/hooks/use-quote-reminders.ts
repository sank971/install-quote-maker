...
import {
  getQuoteReminder, parseReminderSettings, REMINDER_SETTING_KEY, type ReminderSettings,
...
export function useQuoteReminders() {
  ...
  const settingsQuery = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("app_settings")
        .select("value").eq("owner_id", user!.id).eq("key", REMINDER_SETTING_KEY).maybeSingle();
    ...
    queryFn: async () => {
      const page = async (offset: number) => {
        const { data, error } = await supabase.from("quotes")
          .select("id, quote_number, status, sent_at, last_reminded_at, reminder_snoozed_until, reminder_count, reminder_version, client:clients(name)")
          .eq("owner_id", user!.id).in("status", ["envoye", "sent"])
  ...
  const reminders = settings ? (quotesQuery.data ?? []).flatMap((quote) => {
  ...
  const act = useMutation({
    mutationFn: async ({ id, version, action }: { id: string; version: number; action: "done" | "snooze" }) => {
      const { error } = await supabase.rpc("handle_quote_reminder", {
        p_quote_id: id, p_action: action, p_expected_version: version,
      });
      if (error) throw error;
    },
    onSuccess: async (_, variables) => {
      await queryClient.invalidateQueries({ queryKey: ["quotes"] });
      toast.success(variables.action === "done"
        ? "Relance notée. Le prochain rappel est programmé."
        : "Rappel reporté de 3 jours.");
    },
    onError: async () => {
      await queryClient.invalidateQueries({ queryKey: ["quotes"] });
      toast.error("Action impossible. Le devis a peut-être changé ; réessayez après actualisation.");
    },
  });
  return {
    settings, reminders, due, saveSettings, act,
