...
export const Route = createFileRoute("/_authenticated/quote-reminders")({
  component: QuoteRemindersPage,
});
      ...
      <div className="flex flex-wrap items-end gap-3">
        <div className="space-y-2">
          ...
          <Input
            ...
            className="w-32"
            required
            aria-describedby="reminder-delay-help"
          />
        </div>
    ...
    <div className="space-y-6">
        ...
        <CardContent>
          {state.settings ? (
            <ReminderSettingsForm
              ...
              saving={state.saveSettings.isPending}
      ...
      {state.isError ? (
        ...
        <>
                  ...
                  <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-center">
                    <div className="min-w-0 space-y-2">
                      ...
                      <p className="text-xs text-muted-foreground">
                        Envoyé le {new Date(row.sent_at!).toLocaleDateString("fr-FR")} · Rappel le{" "}
                        {new Date(row.dueAt).toLocaleString("fr-FR")}
                      </p>
                      <p className="text-xs text-muted-foreground">
                        {row.reminder_count} relance(s) notée(s)
                        {row.last_reminded_at &&
                          ` · Dernière le ${new Date(row.last_reminded_at).toLocaleDateString("fr-FR")}`}
                      </p>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      <Button variant="outline" size="sm" asChild>
                        <Link to="/quotes/$quoteId" params={{ quoteId: row.id }}>
                          Ouvrir le devis
                        </Link>
                      </Button>
                      {row.isDue && (
          ...
          <p className="text-xs text-muted-foreground">
            Après avoir contacté le client, utilisez « Relance effectuée » pour programmer le rappel
            suivant. Les devis acceptés, refusés ou passés à l’étape suivante sortent
