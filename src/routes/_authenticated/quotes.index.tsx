...
function QuotesList() {
  ...
  const total = (q: any) => {
    ...
    const parts = it.reduce((s: number, i: any) => s + Number(i.unit_price) * Number(i.quantity), 0);
  ...
  return (
    <div>
      <PageHeader
        ...
        actions={<Button asChild><Link to="/quotes/new"><Plus className="mr-2 h-4 w-4" />Nouveau devis</Link></Button>}
      />
      {quotes.length === 0 ? (
        <EmptyState title="Aucun devis" action={<Button asChild><Link to="/quotes/new"><Plus className="mr-2 h-4 w-4" />Créer</Link></Button>} />
      ) : (
                  ...
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        <div className="text-xs text-muted-foreground">{client?.name} · {new Date(q.issued_at).toLocaleDateString("fr-FR")}</div>
                      </div>
                    </div>
